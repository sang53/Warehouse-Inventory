...
import { checkValidation, validateInt, validateOptionalInt, } from "../middlewares/validate.js";
...
export const tasksIDPost = [
    ...
    async (req, res) => {
        const { id, u_id } = matchedData(req);
        const [task] = await FullTask.getFull({ t_id: id });
        // make sure task is valid
        if (task.completed)
            throw new Error(`Task ${String(id)} Already Completed`);
        ...
        if (!taskUser)
            throw new Error("Invalid user to complete task");
        ...
        if (currUser.u_id === taskUser.u_id && currUser.u_role !== "admin")
            ...
            res.redirect(`/tasks/id/${String(id)}`);
    },
];
