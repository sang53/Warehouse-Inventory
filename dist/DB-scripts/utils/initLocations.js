...
const HEIGHT = 9;
const WIDTH = 9;
const LOCATION_TYPES = ["intake", "storage", "outgoing"];
const LOCATIONS_DATA = Array.from({ length: HEIGHT * WIDTH }, (_v, k) => {
    ...
    const l_role = Object.values(LOCATION_TYPES)[Math.floor(row / 3)] ?? LOCATION_TYPES[2];
    return { l_name: `${String(row)}-${col}`, l_role };
});
export default async function (client) {
    await Promise.all(LOCATIONS_DATA.map((locationData) => Location.create(locationData, client)));
}
