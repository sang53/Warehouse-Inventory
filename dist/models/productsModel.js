...
export default class Product {
    ...
    static async getAllStock(order) {
    ...
    ORDER BY ${order};`;
        const { rows } = await db.query(storage_totals + order_totals + query);
        return rows;
    }
    static async getStockByProduct(p_id) {
        const query = `
    SELECT 
      p.p_id,
      p.p_name,
    ...
    WHERE p.p_id = $1;`;
        ...
        const [product] = GeneralModel.parseOutput(rows, `Product ${String(p_id)} Not Found`);
        return product;
    }
    static async getPalletLocation(p_id) {
        const query = `
    ...
    ORDER BY a.stock DESC;`;
        const output = await db.query(query, [p_id]);
        return output.rows;
    }
}
